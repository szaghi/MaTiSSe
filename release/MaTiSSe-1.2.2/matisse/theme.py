...
class Theme(object):
      ...
      for css in self.slide_header[header]:
        for key in css:
          ...
          if 'height' in key.lower():
            found_h = True
      if not found_w:
        self.slide_header[header].append({'width': '100%'})
      if not found_h:
        self.slide_header[header].append({'height': '10%'})
      ...
      for css in self.slide_footer[footer]:
        for key in css:
          ...
          if 'height' in key.lower():
            found_h = True
      if not found_w:
        self.slide_footer[footer].append({'width': '100%'})
      if not found_h:
        self.slide_footer[footer].append({'height': '10%'})
  ...
  def __get_slide_content_width(self):
    ...
    for sidebar in self.slide_sidebar:
      ...
      #     for key in css:
      #       if 'width' in key.lower():
      #         width += int(css[key].strip('%'))
      #         break
    return str(100 - width) + '%'
  ...
  def __check_slide_sidebars_dimensions(self):
    ...
    for sidebar in self.slide_sidebar:
      ...
      if not found_h:
        self.slide_sidebar[sidebar].append({'height': height})
    ...
    for css in self.slide_content:
      for key in css:
        ...
        if 'height' in key.lower():
          ...
          found_h = True
    if not found_w:
      self.slide_content.append({'width': width})
    if not found_h:
      self.slide_content.append({'height': height})
  ...
  def __check_slide(self):
    ...
    for footer in self.slide_footer:
      ...
      self.check_css_attribute(name='clear', value='both', css_list=self.slide_footer[footer])
