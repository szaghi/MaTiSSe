    ...
    if self.theme:
      ...
      for theme in os.listdir(themes):
        if theme == self.theme:
          ...
          theme_css = os.path.join(os.path.join(themes, theme), 'css')
          if os.path.exists(theme_css):
            css = []
            for css_file in os.listdir(theme_css):
              css.append(css_file)
            if len(css) > 0:
              css_overtheme = '---\nmetadata:\n  - css_overtheme:'
              for style in css:
                copyfile(os.path.join(theme_css, style), os.path.join(os.path.join(output, 'css'), style))
                css_overtheme += '\n    - css/' + style
              css_overtheme += '\n---\n'
              source_themed = css_overtheme + source_themed
