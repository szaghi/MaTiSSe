...
__rectn_fig__ = re.compile(r"\$content(\((?P<ctn_type>.*?)\))*(\[(?P<ctn_options>.*?)\])*\{(?P<ctn>.*?)\}", re.DOTALL)
...
class Box(object):
  ...
  def __get_content(self,source):
    ...
    if self.ctn_type == 'figure':
      match = re.search(__rectn_fig__, source)
    else:
...
def parse(source):
  ...
  for match in re.finditer(__rebox__, obfuscate_source):
    ...
    obfuscate_source = re.sub(__rebox__, lambda x: box.to_html(), obfuscate_source, 1)
