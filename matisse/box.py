...
class Box(object):
    ...
    regexs = {
        "box": re.compile(r"(?P<box>\$box(?P<env>.*?)\$endbox)", re.DOTALL),
        "style": re.compile(r"\$style\[(?P<style>.*?)\]", re.DOTALL),
        "content_fig": re.compile(
            r"\$content(\((?P<ctn_type>.*?)\))*(\[(?P<ctn_options>.*?)\])*\{(?P<ctn>.*?)\}", re.DOTALL
        ),
        "content": re.compile(
            r"\$content(\((?P<ctn_type>.*?)\))*(\[(?P<ctn_options>.*?)\])*\{(?P<ctn>.*)\}", re.DOTALL
        ),
        "caption": re.compile(
            r"\$caption(\((?P<cap_type>.*?)\))*(\[(?P<cap_options>.*?)\])*(\{(?P<cap>.*?)\})*", re.DOTALL
        ),
    ...
    def __str__(self):
        string = []
        string.append(f"\nStyle: {self.style}")
        string.append(f"\nCaption({self.cap_type})[options={self.cap_options}]: {self.cap}")
        string.append(f"\nContent({self.ctn_type})[options={self.ctn_options}]: {self.ctn}")
        return "".join(string)
    ...
    def caption_txt(self):
        ...
        if self.cap_type and self.cap:
            txt = f"{self.cap_type} {self.number}: " + markdown2html(source=self.cap, no_p=True)
        elif self.cap_type:
            txt = self.cap_type
        elif self.cap:
    ...
    def to_html(self):
        ...
        doc = Doc()
        with doc.tag("div", id=f"box-{self.number}"):
            if self.style:
