...
def markdown2html(source, no_p=False):
    ...
    if __mdx_checklist__:
        mkd = markdown.Markdown(
            output_format="html5",
            extensions=["smarty", "extra", CustomSpanClassExtension(), ChecklistExtension(), MathJaxExtension()],
        )
    else:
        mkd = markdown.Markdown(output_format="html5", extensions=["smarty", "extra", MathJaxExtension()])
