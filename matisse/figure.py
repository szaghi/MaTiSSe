...
class Figure(Box):
  ...
  def to_html(self):
    ...
    doc = Doc()
    with doc.tag('div', id=f'Figure-{self.number}'):
      if self.style:
      ...
      with doc.tag(self.ctn_type):
        ...
        if self.ctn_options:
          doc.stag('img', src=self.ctn, klass='figure-content', style=self.ctn_options, alt=f'Figure-{self.ctn}')
        else:
          doc.stag('img', src=self.ctn, klass='figure-content', style='width:100%;', alt=f'Figure-{self.ctn}')
