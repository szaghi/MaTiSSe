...
class Columns(object):
    ...
    regexs = {
        "columns": re.compile(r"(?P<columns>\$columns(?P<env>.*?)\$endcolumns)", re.DOTALL),
    ...
    def __str__(self):
        string = []
        if self.number > 0:
            string.append(f"\nColumns number: {self.number}")
            for col, column in enumerate(self.columns):
                string.append(f"\nColumn({col})[options={column[1]}]")
        return "".join(string)
                ...
                for col, column in enumerate(self.columns):
                    with doc.tag("div", klass="column"):
                        ...
                        if column[1]:
                            ...
                            style += f"width:{int(100.0 / self.number)}%;"
